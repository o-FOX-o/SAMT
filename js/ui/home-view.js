...
function occurrenceRow(state, occurrence, controls = true) {
  const relationship = (state.blocks || []).flatMap((block) => block.relationships || []).find((candidate) => candidate.id === occurrence.relationshipId); const canSkip = relationship?.config?.allowSkip === true; const canCompleteManually = relationship?.config?.manualCompletion === true;
  return `<div class="samt-list-row"><div><strong>${esc(occurrenceName(state, occurrence))}</strong><small>${statusLabel(occurrence.status)} · ${esc(fmtDateTime(occurrence.scheduledAt || occurrence.availableFrom))}</small></div>${controls ? `<div class="samt-row-actions"><button class="samt-button primary small" data-action="log-occurrence" data-id="${esc(occurrence.id)}">Log</button>${canCompleteManually ? `<button class="samt-button small" data-action="complete-occurrence" data-id="${esc(occurrence.id)}">Complete</button>` : ""}${canSkip ? `<button class="samt-button ghost small" data-action="skip-occurrence" data-id="${esc(occurrence.id)}" data-require-reason="${relationship?.config?.requireSkipReason === true ? "true" : "false"}">Skip</button>` : ""}</div>` : ""}</div>`;
}
...
export function renderHomeView({ model = {}, state = {} } = {}) {
  ...
  return `<section class="samt-page" aria-labelledby="home-title">
    ...
    <div class="samt-section-grid">
      <article class="samt-card"><div class="samt-card-head"><h2>Avoid</h2><a href="#/actions?direction=avoid">View actions</a></div><div class="samt-list">${(model.avoid || []).length ? model.avoid.map((item) => `<div class="samt-list-row"><div><strong>${esc(item.name)}</strong><small>${esc(statusLabel(item.status || "on_track"))}</small></div><div class="samt-metric"><b>${esc(item.metric === "count" ? String(item.actual || 0) : fmtMinutes(item.actual || 0))}</b><small>${item.metric === "count" ? "logs" : "minutes"}</small></div></div>`).join("") : `<p class="samt-empty">No Avoid Actions configured.</p>`}</div></article>
      <article class="samt-card"><div class="samt-card-head"><h2>Later Today</h2><span class="samt-count">${(model.due?.laterToday || []).length}</span></div><div class="samt-list">${(model.due?.laterToday || []).length ? model.due.laterToday.map((item) => occurrenceRow(state, item, false)).join("") : `<p class="samt-empty">Nothing scheduled later today.</p>`}</div></article>
