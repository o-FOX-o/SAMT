...
function runtimeChildButtons(block, run, child, relation) {
  ...
  if (block.type === "project" && !TERMINAL_CHILDREN.includes(child.state) && child.state !== "LOCKED") output += button("block-project-child", "Block", attrs);
...
function runtimeChildRow(state, block, run, child) {
  ...
  const attrs = " data-run-id=\"" + esc(run.id) + "\" data-relationship-id=\"" + esc(child.relationshipId || child.id) + "\" data-block-id=\"" + esc(block.id) + "\"";
  return "<div class=\"samt-list-row\"><div><strong>" + esc(label) + "</strong><small>" + esc(stateText) + (child.deadline ? " · deadline " + esc(fmtDateTime(child.deadline)) : "") + (child.expectedUnblockAt ? " · expected " + esc(fmtDateTime(child.expectedUnblockAt)) : "") + (child.reason ? " · " + esc(child.reason) : "") + "</small></div><div class=\"samt-row-actions\">" + runtimeChildButtons(block, run, child, relation) + (relation?.kind === "block" ? link("#/blocks/" + encodeURIComponent(relation.refId), "Open") : "") + "</div></div>";
}
...
function cycleRuntime(state, block) {
  ...
  const currentActionId = relationship?.kind === "action" ? relationship.refId : null;
  ...
  const actionControl = currentActionId
    ? button("open-log", "＋ Log", " data-id=\"" + esc(currentActionId) + "\" data-block-id=\"" + esc(block.id) + "\" data-relationship-id=\"" + esc(current.relationshipId) + "\"", "primary") + (relationship.config?.manualCompletion === true ? button("resolve-cycle-slot", "Complete", slotAttrs + " data-outcome=\"completed\"") : "")
    : current ? button("resolve-cycle-slot", "Complete", slotAttrs + " data-outcome=\"completed\"", "primary") : "";
  const skipControl = current && skipAllowed ? button("resolve-cycle-slot", "Skip", slotAttrs + " data-outcome=\"skipped\" data-require-reason=\"true\"") : "";
  const deferControl = current && relationship?.config?.allowDefer === true ? button("resolve-cycle-slot", "Defer", slotAttrs + " data-outcome=\"deferred\"") : "";
  const unavailableControl = current && relationship?.config?.allowUnavailable === true ? button("resolve-cycle-slot", "Unavailable", slotAttrs + " data-outcome=\"unavailable\"") : "";
  return "<article class=\"samt-card samt-runtime-card\"><div class=\"samt-card-head\"><div><h3>Cycle runtime</h3><small>" + esc(titleCase(block.config?.generationMode || big?.generationMode || "simple_ordered")) + " · Small Cycle " + esc(String(small?.smallCycleNumber || 0)) + " · Big Cycle " + esc(String(all.indexOf(big) + 1 || 1)) + "</small></div><div class=\"samt-row-actions\">" + (needsGeneration ? button("generate-small-cycle", "Generate next Small Cycle", " data-id=\"" + esc(block.id) + "\"") : "") + "</div></div><div class=\"samt-detail-grid\"><div><span class=\"samt-eyebrow\">Current</span><h2>" + esc(relationship ? relationshipName(state, relationship) : needsGeneration ? "Generate next" : "No slot") + "</h2><p class=\"samt-muted\">" + (current ? "slot " + (displayIndex + 1) + " of " + slots.length : "No generated slot is available") + "</p></div><div><span class=\"samt-eyebrow\">Next</span><h2>" + esc(next ? relationshipName(state, nextRelationship) : needsGeneration ? "Generate next Small Cycle" : "End of Small Cycle") + "</h2><p class=\"samt-muted\">" + esc(coverage) + "</p></div></div><div class=\"samt-sequence\">" + (slots.length ? slots.map((slot, index) => { const rel = relationshipById(block, slot.relationshipId); return "<span class=\"samt-sequence-item " + (index === displayIndex && current ? "current" : "") + "\">" + (index + 1) + ". " + esc(rel ? relationshipName(state, rel) : slot.relationshipId) + (slot.outcome ? " · " + esc(statusLabel(slot.outcome)) : "") + "</span>"; }).join("") : "<span class=\"samt-muted\">No generated slots yet.</span>") + "</div><div class=\"samt-card-actions\">" + actionControl + skipControl + deferControl + unavailableControl + (canAdvance ? button("advance-cycle", "Next", " data-id=\"" + esc(block.id) + "\"") : needsGeneration ? button("generate-small-cycle", "Generate next Small Cycle", " data-id=\"" + esc(block.id) + "\"") : "") + "</div></article>";
}
function actionListRuntime(state, block) {
  const ids = new Set((block.relationships || []).filter((relationship) => relationship.kind === "action").map((relationship) => relationship.id));
  const now = Date.now();
...
function collectionRuntime(state, block) {
  const descendants = [block.id, ...getDescendantBlockIds(block.id, state.blocks || [])];
  const directIds = new Set((state.actionLogs || []).filter((log) => (log.contextRefs || []).some((reference) => reference.blockId === block.id)).map((log) => log.id));
  const inclusiveIds = new Set((state.actionLogs || []).filter((log) => (log.contextRefs || []).some((reference) => descendants.includes(reference.blockId))).map((log) => log.id));
  return "<article class=\"samt-card samt-runtime-card\"><div class=\"samt-card-head\"><div><h2>Collection activity</h2><p class=\"samt-muted\">Browse children and activity without fake completion or Run controls.</p></div></div><div class=\"samt-detail-grid\"><div><span class=\"samt-eyebrow\">Direct activity</span><strong class=\"samt-stat-value\">" + directIds.size + "</strong><small>unique factual logs</small></div><div><span class=\"samt-eyebrow\">Inclusive unique</span><strong class=\"samt-stat-value\">" + inclusiveIds.size + "</strong><small>this Collection and descendants</small></div></div></article>";
}
function runtimeForBlock(state, block, runs) {
  if (block.type === "collection") return collectionRuntime(state, block);
...
function resultTagOptions(state, selected = "") {
  return (state.tags || []).filter((tag) => ["result", "both"].includes(tag.scope) && tag.status !== "archived").map((tag) => "<option value=\"" + esc(tag.id) + "\" " + (tag.id === selected ? "selected" : "") + ">" + esc(tag.name) + "</option>").join("");
}
function choiceOptionOptions(state, selected = "") {
  return (state.actions || []).flatMap((action) => (action.resultFields || []).filter((field) => field.type === "choice").flatMap((field) => (field.config?.options || []).map((option) => "<option value=\"" + esc(option.id) + "\" " + (option.id === selected ? "selected" : "") + ">" + esc(action.name) + " · " + esc(field.label) + " · " + esc(option.label) + "</option>"))).join("");
}
...
function conditionOptions(config = {}, state = {}, block = {}) {
  const conditions = Array.isArray(config.conditions) && config.conditions.length ? config.conditions : [{ type: "all_required" }];
