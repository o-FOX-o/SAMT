...
export function normalizeResultConfig(field, now = new Date()) {
  ...
  const options = Array.isArray(config.options) ? config.options.map((option, index) => ({ id: option.id || createId("option", now), label: requireName(option.label, "Choice option"), position: Number.isFinite(Number(option.position)) ? Number(option.position) : index, analysisScore: option.analysisScore == null ? null : finiteNumber(option.analysisScore) })) : [];
