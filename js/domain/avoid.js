...
export function evaluateAvoidPeriod({ mode = "binary_limit", actual = 0, allowed = 0, anchors = [], violations = null, penaltyPercent = 100 } = {}) {
  ...
  let lower = sorted[0], upper = sorted.at(-1); for (let i = 0; i < sorted.length - 1; i += 1) if (value >= sorted[i].actual && value <= sorted[i + 1].actual) { lower = sorted[i]; upper = sorted[i + 1]; break; }
