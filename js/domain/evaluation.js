...
function targetProgressForPeriod(target, { logs, period, actions, blocks, units, memo, visiting, resultField = null } = {}) {
  ...
  let result;
  try {
    result = calculateTargetProgress({ target, logs, period, actions, blocks, resultField: resultFieldForTarget(target, actions, resultField), units, childResults });
  } catch (error) {
...
export function evaluatePeriod({ period, target = null, avoid = null, logs = [], actions = [], blocks = [], resultField = null, units = [], now = new Date() } = {}) {
  let evaluation;
  if (target) {
    evaluation = targetProgressForPeriod(target, { logs, period, actions, blocks, units, resultField, memo: new Map(), visiting: new Set() });
  } else if (avoid) {
