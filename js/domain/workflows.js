...
export function createWorkflowStep({ id, relationshipId = null, name = "", required = true, state = "LOCKED", position = 0, timing = null, availabilityMode = "immediate", delayValue = null, delayUnit = "hours", predecessorId = null, availableFrom = null, deadline = null, allowSkip = false, requireSkipReason = false, allowExcuse = false, allowNotApplicable = false, expectedUnblockAt = null, createdAt = null } = {}) {
  ...
  return { id, relationshipId, name: String(name).trim(), required: required !== false, state, position: Math.max(0, Number(position) || 0), timing: clone(timing), availabilityMode, delayValue: delayValue == null ? null : Number(delayValue), delayUnit, predecessorId, availableFrom, deadline, allowSkip: Boolean(allowSkip), requireSkipReason: Boolean(requireSkipReason), allowExcuse: Boolean(allowExcuse), allowNotApplicable: Boolean(allowNotApplicable), expectedUnblockAt, createdAt, resolvedAt: null, reason: null };
}

export function resolveWorkflowStep({ step, state, reason = null, expectedUnblockAt = step?.expectedUnblockAt || null, now = new Date(), allowSkip = step?.allowSkip, allowExcuse = step?.allowExcuse, allowNotApplicable = step?.allowNotApplicable } = {}) {
  ...
  return { ...step, state, reason: reason || null, expectedUnblockAt: state === "BLOCKED" ? expectedUnblockAt || null : state === "AVAILABLE" ? null : step.expectedUnblockAt || null, resolvedAt: terminal ? new Date(now).toISOString() : step.resolvedAt || null, blockedAt: state === "BLOCKED" ? new Date(now).toISOString() : step.blockedAt || null };
}

export function transitionWorkflowStep({ run, stepId, state, reason = null, expectedUnblockAt = null, now = new Date() } = {}) {
  ...
  const nextStep = resolveWorkflowStep({ step: before, state, reason, expectedUnblockAt, now }); const steps = run.steps.map((step, current) => current === index ? nextStep : step);
  ...
  const stamp = new Date(now).toISOString(); const transitions = [...(run.transitions || []), { type: "STEP_STATE", stepId, from: before.state, to: state, reason: reason || null, expectedUnblockAt: state === "BLOCKED" ? expectedUnblockAt || null : null, at: stamp }];
