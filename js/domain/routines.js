...
export function updateRoutineChild({ run, relationshipId, state, logId = null, reason = null, now = new Date() } = {}) {
  ...
  const availableAt = child.availableFrom && Number.isFinite(new Date(child.availableFrom).getTime()) ? new Date(child.availableFrom) : null;
  if (child.available === false || availableAt && new Date(now) < availableAt) throw new ValidationError("This Routine child is not available yet.");
